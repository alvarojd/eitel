...
export function MapPicker({ initialLat, initialLng, onSelect, onClose }: MapPickerProps) {
 ...
 <p className="text-[10px] text-slate-500 tracking-widest font-bold mt-0.5">Define la ubicación exacta del sensor en el mapa</p>
 </div>
 </div>
 ...
 <p className="text-[9px] font-black text-slate-600 tracking-tighter">Latitud</p>
 <p className="text-sm font-mono font-black text-sky-400">{position ? position[0].toFixed(6) : '---'}</p>
 </div>
 <div className="px-4 py-2 text-right">
 <p className="text-[9px] font-black text-slate-600 tracking-tighter">Longitud</p>
 ...
 className="bg-sky-500 text-white px-6 py-3 rounded-full shadow-2xl text-xs font-black tracking-widest flex items-center gap-3 border-2 border-white/20"
 >
 ...
 <button 
 onClick={onClose}
 className="text-sm font-black text-slate-400 hover:text-white tracking-widest transition-colors"
 >
 Descartar
 </button>
 ...
 className="flex items-center gap-3 bg-sky-600 hover:bg-sky-500 disabled:bg-slate-800 disabled:text-slate-600 text-white px-10 py-5 rounded-2xl font-black text-sm tracking-widest shadow-2xl shadow-sky-900/20 transition-all active:scale-95 group"
 >
