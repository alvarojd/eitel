...
import { Thermometer, Droplets, Wind, MapPin, Cpu } from 'lucide-react';
...
export function DeviceReport({ sensor, data, presenceFilter }: DeviceReportProps) {
  ...
  const pieData = useMemo(() => {
    ...
    return [
      ...
      { name: 'Situación Ideal', value: green, color: '#10b981' },
  ...
  if (totalHours === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-64 text-white/60 print:text-slate-600 print:text-slate-600 bg-slate-900/20 rounded-2xl border border-slate-800/50">
        ...
        <p className="text-sm font-medium">No hay datos suficientes para el dispositivo seleccionado.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        {/* Device Info Card */}
        <div className="col-span-1 bg-slate-900/40 border border-slate-800/60 p-5 rounded-2xl shadow-xl flex flex-col justify-center print:bg-white print:border-slate-300">
            <h3 className="text-lg font-bold text-white truncate print:text-black  ">{sensor.name}</h3>
            <p className="text-[10px] text-white/60 print:text-slate-600 print:text-slate-600 font-mono mt-1 truncate">{sensor.id}</p>
            <div className="mt-4 flex items-start gap-2 text-xs text-white/80">
              ...
              <div>
                {sensor.latitude && sensor.longitude ? (
                   <a 
                     ...
                     className="text-[11px] font-bold text-sky-500 hover:text-sky-400 transition-colors mt-0.5 block"
                   >
                     {sensor.latitude}, {sensor.longitude}
                   </a>
                ) : (
                  <span className="text-[11px] font-bold mt-0.5 block">Sin ubicación GPS</span>
                )}
              </div>
            </div>
            <div className="mt-4 text-[10px] font-bold text-white/60 print:text-slate-600 print:text-slate-600  border-t border-slate-800/60 pt-3 ">
