...
const NavItem = ({ icon, label, active = false }: { icon: React.ReactNode, label?: string, active?: boolean }) => (
  <div className={`p-3 rounded-xl mb-2 cursor-pointer transition-all duration-200 group flex items-center justify-center lg:justify-start ${active ? 'bg-sky-600 text-white shadow-lg shadow-sky-900/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>
    {icon}
    ...
    <div className="h-screen w-screen bg-slate-900 flex overflow-hidden">
      ...
      <nav className="w-16 lg:w-64 bg-slate-850 h-full flex flex-col border-r border-slate-800 z-10 flex-shrink-0">
        ...
        <div className="flex-1 p-3">
          <NavItem icon={<Home size={20} />} label="Resumen" active />
          <NavItem icon={<Layers size={20} />} label="Dispositivos" />
